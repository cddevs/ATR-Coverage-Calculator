...
def calculate(height, scan_angle, scan_angle_use):
    if check_input_number(scan_angle):  
        height = float(height)
        spacing = 2 * math.tan(math.radians(scan_angle)) * height
...
# https://keisan.casio.com/exec/system/1273849674 - Right triangle Calculator
# Knowing angle and height, base = h * tan(angle)
# math.tan(math.radians(scan_angle)) * height
...
def coverage():
    while True:
        ...
        if check_input_number(user_input):
            calculate(user_input, 60, "default")
            calculate(user_input, 54.7, "recommended location accuracy")
            break
        else:
